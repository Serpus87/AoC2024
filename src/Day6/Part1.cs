...
public static class Part1
{
    ...
    /// 
    /// Safety protocols clearly indicate that new pages for the safety manuals must be printed in a very specific order.The notation X|Y means 
    /// that if both page number X and page number Y are to be produced as part of an update, page number X must be printed at some point before page number Y.
    /// 
    /// The Elf has for you both the page ordering rules and the pages to produce in each update (your puzzle input), but can't figure out whether 
    ...
    /// number 47 and page number 53, then page number 47 must be printed at some point before page number 53. (47 doesn't necessarily need 
    /// to be immediately before 53; other pages are allowed to be between them.)
    /// 
    ...
    /// page number, and so only some of the ordering rules apply - within each update, the ordering rules that involve missing page numbers are not used.
    /// 
    ...
    /// the correctly-ordered updates, you will need to find the middle page number of each correctly-ordered update. In the above example, the 
